...
export default class BookRepository {
    ...
    async addReviewToBook(bookId, text, rating) {
        // add rating object to reviews collection.....
        const newReview = new reviewModel({ text, rating, bookId });

        const addedReview = await newReview.save();

        // add ratingId to the book document....

        await booksModel.updateOne({ _id: bookId }, { $push: { reviews: addedReview._id } });
