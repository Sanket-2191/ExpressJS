...
export default class BookRepository {
    ...
    async addReviewToTarget(targetId, target, text, rating) {
        ...
        if (target === "Author") {
            reviewAdded = await authorsModel.findByIdAndUpdate(targetId, { $push: { reviews: addedReview._id } })


        } else {
            reviewAdded = await booksModel.findByIdAndUpdate(targetId, { $push: { reviews: addedReview._id } })
        ...
        if (reviewAdded) {
            ...
            throw new Error("The specified target entity (Author/Book) was not found.")
        };
    }
