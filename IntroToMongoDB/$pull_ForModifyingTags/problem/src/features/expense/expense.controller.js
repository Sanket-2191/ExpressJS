...
export default class ExpenseController {
  ...
  updateTag = async (req, res) => {
    ...
    try {
      ...
      const result = await this.expenseRepository.updateTagInExpense(id, oldTag, newTag);
      console.log(result);
      res.status(200).send("Tags updated successfully.");
    } catch (err) {
      console.log(err);
      res.status(503).send("OPPS")
    }
  };
  ...
  deleteTag = async (req, res) => {
    try {
      ...
      console.log({ id, tag });
      const result = await this.expenseRepository.deleteTagFromExpense(id, tag);
      console.log(result);
      res.status(200).send("Tags deleted successfully.");
    } catch (err) {
      console.log(err);
      res.status(503).send("OPPS")
    }
